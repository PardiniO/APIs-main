export class LibroDetalle {
    static mostrar(libro) {
        ...
        const autorLink = document.getElementById('autor-libro').querySelector('a');
        if (autorLink) {
            autorLink.textContent = libro.autor || 'Autor desconocido';
            autorLink.href = libro.enlace || '#';
            autorLink.onclick = (e) => {
                e.preventDefault();
                if (libro.infoExtra && libro.infoExtra.autorID) {
                    window.location.href = `autor.html?id=${libro.infoExtra.autorID}`;
                } else {
                    window.location.href = `autor.html?nombre=${encodeURIComponent(libro.autor || 'Autor desconocido')}`;
                }
            };
        }
        ...
        if (data.authors && data.authors.length > 0 && data.authors[0].key) {
            autorID = data.authors[0].key.replace('/authors/', '');
        }
        return new LibroInfo({
            autor: data.authors && data.authors[0] ? data.authors[0].name : 'Autor desconocido',
            infoExtra: {
                autorID: autorID
            }
        });
